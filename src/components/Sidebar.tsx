...
import { CollapsibleOptionList } from './CollapsibleOptionList';
...
/** Icon (or avatar) node for one instant-search / default source entry. */
function sourceIcon(source: { icon: string | null; type: string }) {
    if (source.icon) {
        return <img src={source.icon} alt="" className="h-4 w-4 shrink-0 rounded-full object-cover" />;
    }
    const { icon: TypeIcon, className } = SOURCE_TYPE_META[source.type] ?? DEFAULT_SOURCE_TYPE_META;
    return <TypeIcon className={`h-3.5 w-3.5 shrink-0 ${className}`} />;
}
...
export function Sidebar({
    ...
    // the trigger does *not* expand it. See CollapsibleOptionList for the
    // shared open/focus/blur behavior all five of these panels share.
    ...
    // trigger/list was focused (well after the rAF refocus in the onSelect
            ...
            >
                ...
                {!query.q && !query.source && (activeProviderInfo?.defaultSources.length ?? 0) > 0 && (
                    <CollapsibleOptionList
                        ...
                        onOpen={() => setSuggestedOpen(true)}
                        onClose={() => setSuggestedOpen(false)}
                        triggerIcon={<Sparkles className="h-4 w-4 shrink-0" />}
                        triggerLabel="Suggested sources"
                        triggerClassName="mb-5 text-(--color-text-dim) hover:text-(--color-text)"
                        listClassName="mb-5 max-h-36"
                        triggerRef={suggestedTriggerRef}
                        listRef={suggestedListRef}
                        options={activeProviderInfo!.defaultSources.map((s) => ({
                            key: s.slug,
                            label: s.name,
                            icon: sourceIcon(s),
                            onSelect: () => {
                ...
                {suggestions && suggestions.queries.length > 0 && (
                    <CollapsibleOptionList
                        isOpen={queriesOpen}
                        onOpen={() => setQueriesOpen(true)}
                        onClose={() => setQueriesOpen(false)}
                        triggerIcon={<Search className="h-4 w-4 shrink-0" />}
                        triggerLabel="Related searches"
                        triggerClassName="mb-2 text-(--color-text-dim) hover:text-(--color-text)"
                        listClassName="mb-2 max-h-36"
                        triggerRef={queryTriggerRef}
                        listRef={queryListRef}
                        options={suggestions.queries.map((q) => ({
                            key: q,
                            label: q,
                            icon: <Search className="h-3.5 w-3.5 shrink-0 text-(--color-text-dim)" />,
                            onSelect: () => {
                                setSearchText(q);
                ...
                {suggestions && suggestions.sources.length > 0 && (
                    <CollapsibleOptionList
                        ...
                        triggerIcon={<Users className="h-4 w-4 shrink-0" />}
                        triggerLabel="Sources"
                        triggerClassName="mb-5 text-(--color-text-dim) hover:text-(--color-text)"
                        listClassName="mb-5 max-h-48"
                        triggerRef={sourceTriggerRef}
                        listRef={sourceListRef}
                        options={suggestions.sources.map((s) => ({
                            key: s.slug,
                            label: s.name,
                            icon: sourceIcon(s),
                            onSelect: () => {
              ...
              Source: <span className="text-(--color-text)">{query.source}</span>
                ...
                {orderParam?.enum && (
                    <>
                        ...
                        <CollapsibleOptionList
                            ...
                            triggerLabel={query.order ?? orderParam.default}
                            triggerClassName="mb-5 capitalize text-(--color-text) hover:bg-white/5"
                            listClassName="mb-5 max-h-48"
                            optionClassName="capitalize"
                            triggerRef={orderTriggerRef}
                            listRef={orderListRef}
                            options={orderParam.enum.map((opt) => ({
                                key: opt,
                                label: opt,
                                selected: (query.order ?? orderParam.default) === opt,
                                onSelect: () => {
                ...
                {availableFlairs && availableFlairs.length > 0 && (
                    <>
                        ...
                        <CollapsibleOptionList
                            isOpen={flairOpen}
                            onOpen={() => setFlairOpen(true)}
                            onClose={() => setFlairOpen(false)}
                            triggerLabel={query.flair ?? 'All'}
                            triggerClassName={`hover:bg-white/5 ${query.flair ? 'text-(--color-text)' : 'text-(--color-text-dim)'}`}
                            listClassName="mb-5 max-h-48"
                            triggerRef={flairTriggerRef}
                            listRef={flairListRef}
                            options={[
                                {
                                    key: '__all__',
                                    label: 'All',
                                    selected: !query.flair,
                                    onSelect: () => {
